...
class FlutterAutofillService : AutofillService() {
    ...
    override fun onCreate() {
        ...
        (org.tinylog.provider.ProviderRegistry.getLoggingProvider() as? DynamicLevelLoggingProvider)?.let { provider ->
    ...
    override fun onFillRequest(
        ...
        val pendingPrefs = getSharedPreferences("com.keevault.flutter_autofill_service.pending", Context.MODE_PRIVATE)
        val pendingPackage = pendingPrefs.getString("pending_package", null)
        val pendingUsername = pendingPrefs.getString("pending_username", null)
        val pendingPassword = pendingPrefs.getString("pending_password", null)

        if (pendingPackage != null && pendingUsername != null && pendingPassword != null) {
            if (parser.packageNames.contains(pendingPackage)) {
                logger.info { "Found pending login for $pendingPackage. Injecting directly." }
                
                // Clear the pending login now that we've found it
                ...
                // We need to find the specific autofill IDs to fill
                val (autoFillIdPasswordGuessed, autoFillIdUsernameGuessed) = SaveHelper.guessAutofillIdsForSave(parser)
                
                if (autoFillIdUsernameGuessed != null) {
                    datasetBuilder.setValue(autoFillIdUsernameGuessed, AutofillValue.forText(pendingUsername))
                    fieldsFound = true
                }
                
                if (autoFillIdPasswordGuessed != null) {
                    datasetBuilder.setValue(autoFillIdPasswordGuessed, AutofillValue.forText(pendingPassword))
                    fieldsFound = true
                }

                if (fieldsFound) {
                    try {
                        callback.onSuccess(fillResponseBuilder.addDataset(datasetBuilder.build()).build())
                        return
                    } catch (e: Exception) {
                        logger.error { "Error building pending dataset: ${e.message}" }
                    }
                } else {
                    logger.warn { "No fields found to inject pending login for $pendingPackage" }
                }
            }
        }
        ...
        val activityName = (packageManager.getServiceInfo(self, PackageManager.GET_META_DATA).metaData?.getString("com.keevault.flutter_autofill_service.ACTIVITY_NAME")
            ?: packageManager.getApplicationInfo(packageName, PackageManager.GET_META_DATA).metaData?.getString("com.keevault.flutter_autofill_service.ACTIVITY_NAME"))
    ...
    override fun onSaveRequest(request: SaveRequest, callback: SaveCallback) {
        ...
        val self = ComponentName(this, javaClass)
        logger.trace { "Trying to fetch package info." }
        val activityName = (packageManager.getServiceInfo(self, PackageManager.GET_META_DATA).metaData?.getString("com.keevault.flutter_autofill_service.SAVE_ACTIVITY_NAME")
            ?: packageManager.getApplicationInfo(packageName, PackageManager.GET_META_DATA).metaData?.getString("com.keevault.flutter_autofill_service.SAVE_ACTIVITY_NAME")
            ?: packageManager.getServiceInfo(self, PackageManager.GET_META_DATA).metaData?.getString("com.keevault.flutter_autofill_service.ACTIVITY_NAME")
            ?: packageManager.getApplicationInfo(packageName, PackageManager.GET_META_DATA).metaData?.getString("com.keevault.flutter_autofill_service.ACTIVITY_NAME"))
            ?: "com.keevault.flutter_autofill_service_example.AutofillActivity"
        logger.debug { "got activity $activityName" }
